...
def _fetch_paginated_items(
    repository: str, query_builder: callable, data_path: list[str], parser: callable
) -> dict[str, dict]:
    """
    ...
    while has_next_page:
        try:
            ...
            for key in data_path:
                if current is None:
                    print(
                        f"Error: Invalid response from GraphQL API for {repository}",
                        file=sys.stderr,
                    )
                    return items
    ...
    try:
        ...
        while has_next_page:
            ...
            if "data" not in data or not data["data"]["repository"] or not data["data"]["repository"][type_key]:
                print(
                    f"Error: Invalid response from GraphQL API for {repository}#{number}",
                    file=sys.stderr,
                )
                break
    ...
    for issue_dir in repo_path.iterdir():
        ...
        if timestamp_file.exists():
            try:
                ...
                print(
                    f"Error reading timestamp file {timestamp_file}: {e}",
                    file=sys.stderr,
                )
        ...
        for number, item_data in open_items.items():
            ...
            if not item_dir.exists():
                ...
                if dry_run:
                    ...
                    print("    [DRY RUN] Would save .last_checked timestamp")
    ...
    for repository, issue_numbers in issues_by_repo.items():
        ...
        for number in issue_numbers:
            ...
            if number in open_items:
                ...
                if item_type == "pr":
                    if has_update:
                        print("    PR has been updated, emitting update event")
                        event_subject = "github.pr.updated"
                    else:
                        print("    PR is still open (no updates since last check)")
                        event_subject = None
                else:
                    if has_update:
                        print("    Issue has been updated, emitting update event")
                        event_subject = "github.issue.updated"
                    else:
                        print("    Issue is still open (no updates since last check)")
                        event_subject = None
                ...
                event_data = {"repository": repository, "number": number}
...
async def monitor_pr_comments(js, base_path: Path, active_prs: list[tuple[str, str]], dry_run: bool = False) -> int:
    """
